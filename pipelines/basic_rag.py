...
specific filing or recent event, it might hallucinate. Pipeline 2 gives
...
relationships. If the answer requires connecting Company A -> Filing B ->
Topic C, vector search won't find that chain. It just returns the 5 most
textually similar chunks, which might miss the connection entirely.
"""
...
def build_vector_store(force_rebuild: bool = False):
    ...
    Also reports one-time ingestion cost (embedding time/count) separately
    from per-question inference costs, per round 3 requirements.
    """
    ...
    collection = client.create_collection(
        ...
        metadata={"description": "sp100 SEC filings for GraphRAG hackathon round 3"},
    )
    ...
    ingestion_report_path = os.path.join(results_dir, "basic_rag_ingestion_cost.json")
...
def run_query(question: str, collection=None, model=None) -> QueryMetrics:
    ...
    ctx_tokens = count_context_tokens(prompt, question) if prompt else 0

    # Retrieved evidence, required by round 3 (retrieved document/chunk
    # identifiers + evidence actually supplied to the LLM)
    evidence = [
        {"source_id": chunk["chunk_id"], "text": chunk["text"]}
